...
compare_ranks <- function(
  ...
  metadata_cols <- c("Drug1", "Drug2", "Cell_Line", "Tissue")
  feats <- setdiff(names(data), metadata_cols)
  data$Combination <- apply(data[feats], 1, paste, collapse = "_")
  
  cl <- makeCluster(cores)
  on.exit(stopCluster(cl), add = TRUE)
  clusterEvalQ(cl, { library(Matrix); library(RobustRankAggreg); library(BayesMallows) })
  ...
  run_bayesmallows <- function(mat, combs) {
    ...
    bm_data <- setup_rank_data(rank_mat)
    model_opts <- set_model_options(metric = "kendall")
    compute_opts <- set_compute_options(nmc = 5000, burnin = 1000)
    priors <- set_priors()
    inits <- set_initial_values()
    progress <- set_progress_report(verbose = FALSE)
    model <- compute_mallows(
      data = bm_data,
    ...
    cp <- compute_consensus(model, type = "CP")
  ...
  run_cemc_k <- function(mat, combs) {
    ...
    kendals <- sapply(seq_len(ncol(rk)), function(k) {
      ...
      mean(apply(rk, 2, function(fr) cor(vals, fr, method = "kendall", use = "complete.obs")), na.rm = TRUE)
    })
  ...
  for (n in sample_sizes) {
    ...
    idx <- if (n == nrow(data)) seq_len(nrow(data)) else sample.int(nrow(data), n)
    sub <- data[idx, ]
    labels <- synergy_labels[idx]
    ...
    for (m in names(methods)) {
      ...
      t0 <- system.time({
        out[[m]] <- tryCatch(
          methods[[m]](mat, combs),
          error = function(e) tibble(Combination = combs, Score = NA_real_, Rank = NA_integer_)
        )
      })["elapsed"]
    ...
    combined <- reduce(
      map2(valid, names(valid), ~ .x %>% select(Combination, Rank) %>% rename(!!.y := Rank)),
      inner_join, by = "Combination"
    )
    
    combined <- inner_join(
      sub %>% select(Combination, Drug1, Drug2, Cell_Line, Tissue),
      combined, by = "Combination"
    ) %>% select(Drug1, Drug2, Cell_Line, Tissue, everything(), -Combination)
