...
compare_ranks <- function(
  ...
  for (n in sample_sizes) {
    ...
    combined <- inner_join(
      sub %>% select(Combination, Drug1, Drug2, Cell_Line, Tissue, SynergyRank, all_of(feats)),
      combined,
      by = "Combination"
    ) %>%
      select(Drug1, Drug2, Cell_Line, Tissue, SynergyRank, all_of(feats), everything(), -Combination)
    ...
    method_corr <- purrr::map_dbl(method_names, ~
                                    cor(combined$SynergyRank, combined[[.x]],
                                        method = "spearman", use = "complete.obs")
    )
    
    feat_corr <- purrr::map_dbl(feat_names, ~
                                  -cor(combined$SynergyRank, combined[[.x]],
                                       method = "spearman", use = "complete.obs")
    )
    
    all_corr <- tibble(
      Variable    = c("SynergyRank", method_names, feat_names),
      SpearmanRho = c(1,                method_corr, feat_corr)
    )
    
    write_csv(
      all_corr,
      file.path(folder, paste0("Spearman_AllCorr_", n, ".csv"))
    )
  }
